...
Level7to5=function(x){
  ...
  if(x %in% c('BLL','COD','HAD','RED','REG','REB','REN','HKS','HKR','PLA','WIT','YEL','GHL','HAL','FLW','FLS','FLD','FLX','FLY','ANG','SRA','TOM','ANT','CUN','USK','GRC','BLI','LIN','LUM',
              ...
              'RJM','MON','RJN','RJE')) x <- "DEF" # "Demersal_fish"  # FAO sp code

  if(x %in% c('ABR','HLT','KRS','MUS','KNK','KMS','SNE','MOL','CLR','CLH','CLQ','CLS','CLT','COC','SCB','SCC','ISC','SCA','SCX','OYA','WHX','PER','WHE','QSC','VSC','MYV','VEV','GKL','SCE','OYF','OYG')) x <- "MOL" # "Molluscs"  # FAO sp code
  ...
  if(x %in% c('SQL','SQI','SQU','OCC','CTC','SQR','IAX','SQZ','OMZ','CTL')) x <- "CEP" # "Cephalopods"
