...
CompClassiftoOrdi=function(dat, Step2, clusters){
...
#  barplot(table(logSpeciesL5), main="Number of logevents by group of species level 5", xlab="Groups of species level 5", ylab="Number of logevents")
  ...
  datL5=dat
  colnames(datL5)=sapply(as.list(colnames(datL5)), Level7to5)   ## quand on aura toutes les correspondances espèces vers groupes d'espèces, on pourra utiliser Level5 à la place de Level7to5.
  ...
  ###########################################
  # LEVELS 7 AND 5 FOR METIERS FROM CLASSIF #
  ###########################################
  ...
  # Metiers (level 5.7) of each logevent found thanks to the classif_step3 method.
  ...
  # Compare the metiers of classif_Step3 (level 5.7) vs the first group of species in catch (level 5)
  ...
  # Compare the metiers (level 5.7) found thanks to the classif (Step3) vs the first species in catch (level 5)
