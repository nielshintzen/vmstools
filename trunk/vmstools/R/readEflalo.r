readEflalo <- function(file,sep=",",dec="."){
                  ...
                  if(length(grep("KG",colnames(eflalo))) < 1) stop("Units of landings are not in KG")
                  if(length(grep("LE_KG",colnames(eflalo))) < 1) stop("No landing data provided")
                  if(length(grep("LE_EURO",colnames(eflalo))) < 1) warning("Currency used is different than 'EURO'")
