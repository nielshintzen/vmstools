...
% Please edit documentation in R/nestedGrid.r
\name{polyDef}
\alias{polyDef}
\title{Define polygon in text string format}
\usage{
polyDef(lon, lat, gridx, gridy)
}
\arguments{
\item{lon}{Vector with longitude (or x) values}

\item{lat}{Vector with latitude (or y) values, should have the same length
as \code{lon}}

\item{gridx}{gridcell size in degrees in the longitude / x direction}

\item{gridy}{gridcell size in degrees in the latitude / y direction}
}
\value{
A Well Known Text string for each value of \code{lon} and
\code{lat}.
}
\description{
...
\note{
Any points that lie exactly on the border between two grid cells will
