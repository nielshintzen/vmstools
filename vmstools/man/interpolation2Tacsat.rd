...
% Please edit documentation in R/interpolation2Tacsat.r
\name{interpolation2Tacsat}
\alias{interpolation2Tacsat}
\title{Convert the output (a list) from interpolateTacsat to tacsat}
\usage{
interpolation2Tacsat(interpolation, tacsat, npoints = 10, equalDist = TRUE)
}
\arguments{
\item{interpolation}{A list of tracks between actual pings output by the
interpolateTacsat function.}

\item{tacsat}{Tacsat data used when the interpolateTacsat function was
called.}

\item{npoints}{The number of pings or positions required between each real
or actual vessel position or ping}

\item{equalDist}{Logical. Whether the number of positions returned should be
equally spaced or not}
}
\value{
A data frame in tacsat format is returned with estimated locations,
