...
DCFIndicator6 <- function ( tacsat,
  ...
  for (x in 1:length(ltMonth)){
    ...
    monthlyVmsGrid<-vmsGridCreate(monthlyTacsat, nameLon = "SI_LONG", nameLat = "SI_LATI", cellsizeX=cellresX, cellsizeY=cellresY, nameVarToSum = "INMCP", plotMap=plotMapTF, plotTitle=paste("Month ", currMonth), plotPoints = FALSE, outGridFile = paste("DCF6UKVMS2008_", currMonth,".asc",sep=""))
