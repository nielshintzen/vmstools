    ...
    png(paste(analysisName,"_HAC_Dendogram.png",sep="_"), width = 1200, height = 800)
     ...
     while (( fait == FALSE) && (nb_cut < (p-nb.finalclusters))) {
      ...
      print(paste("-----------   nb_cut = ",nb_cut))
       ...
       print(paste("-------------  ",unique(cah_cluster_var_step[namesResidualSpecies])))
     ...
     rect.hclust(cah_var, k=(nb.finalclusters+1))
...
}  else { NamesMainSpeciesHAC <- NA;nbMainSpeciesHAC<-as.numeric(NA) }
