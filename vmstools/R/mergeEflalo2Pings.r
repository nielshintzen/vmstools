...
#' Coupling of VMS and logbooks data including landings dispatching between
#' fishing pings.
#' 
#' The first step of the merging routine consisted of assigning a common
#' identifier joining each VMS trip to a logbook trip by searching for the
#' nearest trip among all logbook trips based on the temporal mid-point of
#' trips. This circumvents problems where the start and end time of VMS and
#' logbook trips do not match exactly and ensure that each logbook trip will be
#' merged with a given VMS trip even if the reporting of the start and end date
#' by fishermen is uncertain. We observed in the data that this reporting date
#' may fluctuate with +/- 1 day around the VMS trip dates possibly because of
#' error in reporting. Additionally, the possible few remaining logbook trips
#' were also linked with the nearest VMS trips. This latter event may occur
#' from unknown failure in the VMS device while fishermen were still fishing.
#' In the processing, links were then forced to nearest VMS trips to prevent
#' loss of any reported landings.
#' 
#' After this fist merging, the distinction of fishnig and not-fishing states
#' can be performed if the 'detectFishing' argument is at TRUE. The current
#' default method ('segment') will run an automatic detection of the fishing
#' peak given the gear type in use. After having detected the vessel-specific
#' speed lower and upper boundaries, one state (i.e. in SI_STATE) is assigned
#' to each ping.
#' 
#' The second step of the process evaluated the consistency between both types
#' of data sources. Sequentially, a quality flag (i to iv) was issued for each
#' recorded position depending on the various degrees of matching, from finer
#' to coarser, if both data have in common: (i) the trip identifier, the same
#' area (ICES rectangle), and the catch date (ii) only the trip identifier and
#' the area, or (iii) only the trip identifier. (the flag iv occur when not
#' fishing points is remaining while some landings are still to be allocated in
#' these very few cases, all steaming points are actually converted to fishing
#' points). The initial datasets (both VMS and logbooks) were split into
#' sub-blocks according to this flag. Each pair of sub-blocks was then joined
#' separately and all merged sub-blocks were bound in one dataset afterwards.
#' 
#' In a final step, declared landings (in weight and value) at the ICES
#' rectangle scale from logbooks were further allocated at a lower spatial and
#' time scale level with regard to the VMS information.  This was done by
#' allocating a proportion of the landings to each detected fishing position
#' depending on the level of matching between VMS and logbook data as described
#' in step 2. In the case of full matching, landings allocated to a given
#' position were proportional to the number of detected fishing positions lying
#' inside each declared logbook area for a given date, assuming that the total
#' landings declared in this area at this date are divided equally among all of
#' these positions. In the particular case of the use of several gear types
#' within the same rectangle and also for the same catch date, an equal share
#' of landings between gear types was also assumed for this day. In case of
#' partial matching, the following procedure was used. First, when the match
#' was correct by area but not by catch date, the landings were equally
#' dispatched between all the fishing positions of this trip detected in this
#' particular ICES rectangle. Second, when the match failed on both catch date
#' and area, the landings concerned were equally allocated to all the detected
#' fishing positions of the trip for which no previous match has been detected.
#' Finally, residual landings (kept in the final output) occurred when the
#' match failed both for the catch date and area, and no fishing position
#' remained.
#' 
#' Warning: However, one should take care that the generated maps related to
#' the origin of landings do not entirely reflect the spatial distributions of
#' the harvested populations but more accurately should be interpreted as the
#' conjunction between the stocks and the fishermen effort distribution.  The
#' distribution of the landings on discrete positions should also be understood
#' with some cautions ( albeit the number of pings may be high enough to be
#' considered representative on a large part of the total exploited areas) and
#' possible allocation of landings on locations we know that a stock is not
#' present might potentially occur.
#' 
#' in tacsat SI_STAT, fishing should be coded 1 and non-fishing should be coded
#' 2. The method is very sensitive to missing harbours. Some warning messages
#' are then provided to help the user to detect some repeated positions that
#' are likely to be missing harbours.  In this last case you should revise the
#' harbour list before processing again.
#' 
#' @param eflalo data.frame, eflalo format
#' @param tacsat data.frame, tacsat format
#' @param general list, general settings to go through all sub-functions e.g.
#' detectFishing
#' @param vessels (optional) name of the vessel(s) to process e.g. those in
#' tacsat
#' @return Nothing is returned but a merged data.frame per vessel in the output
#' folder.  These data.frame could be later bound into a big one using
#' bindAllMergedTable()
