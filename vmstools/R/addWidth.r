...
for(i in 1:length(interpolation)){
  ...
  if(all(is.na(xs))==F & all(is.na(ys))==FALSE){
    ...
    for(j in 1:(nrow(outpointr)-1)){
      singlePolygons[[j]]   <- st_polygon(list(cbind(c(outpointr[j,1],outpointl[j,1],outpointl[j+1,1],outpointr[j+1,1],outpointr[j,1]),
                                             c(outpointr[j,2],outpointl[j,2],outpointl[j+1,2],outpointr[j+1,2],outpointr[j,2]))))
    }
    allPolygons[[counter]] <- st_multipolygon(singlePolygons)
  }
}
return(st_as_sfc(allPolygons))}
