#' convert the merged (VMS + logbooks) data.frame to comply with the fishframe
#' format
#' 
#' the merged data.frame coming from mergeEflalo2Pings() can be further
#' converted to be uploaded in the fishframe data warehouse. This includes the
#' aggregation of fishing pings (VMS effort and logbook landings) by CSquare
#' area code and month.
#' 
#' The function first starts by searching the input data.frame in the
#' output.path folder. The result will be saved in the output.path folder as
#' well. the 'data.table' package is required to perform the aggregation of the
#' data.frame much faster. An area code is also added (e.g. from the ICESarea()
#' function). This latter step is costly in terms of computation time due to
#' the spatial polygon inclusion test performed for a lot of VMS positions
#' (across a lot of polygons!) and this explains the current poor time
#' performance of the whole function.
#' 
#' @param general a list with general settings, e.g. the output.path
#' @param a.year a year to be inserted in the csv
#' @param a.country a country to be inserted in the csv
#' @param degree XY grid resolution in degree
