...
for(iStep in 1:(dim(VMS.)[1]-1)){
  ...
  if(endDataSet != 2){
    ...
    if(method == "cHs" & int == 1){
      ...
      Hx0 <- Hx0 * params$fm * lonLatRatio(VMS.[c(startVMS,endVMS),"declon"],VMS.[c(startVMS,endVMS),"declat"])[1]
      Hx1 <- Hx1 * params$fm * lonLatRatio(VMS.[c(startVMS,endVMS),"declon"],VMS.[c(startVMS,endVMS),"declat"])[2]
      Hy0 <- Hy0 * params$fm
      Hy1 <- Hy1 * params$fm
